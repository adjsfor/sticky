...
public class CommentView extends VerticalPanel implements KeyDownHandler,
		Model.DataObserver {
	...
	public void onKeyDown(KeyDownEvent event) {
		...
		if (keyCode == KeyCodes.KEY_ENTER && !event.isShiftKeyDown()) {
			...
			final Comment comment = new Comment(this.note.getKey(), this.model
					.getCurrentAuthor().getName(), text);
			this.addComment(comment);
			this.comments.add(comment);
			this.model.updateNoteContent(this.note, this.comments, Transformation.NONE);
		}
	}
	...
	private void addComment(Comment comment) {
		...
		this.tblComments.setText(count, 0,
				Author.getShortName(comment.getAuthor()) + ": ");
