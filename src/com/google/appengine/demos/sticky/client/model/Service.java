...
public interface Service extends RemoteService {
	 ...
	 * out for the user (i.e. putting notes on someone else's surface).
	 */
	...
	static class GetNotesResult implements Serializable {
		...
		/**
		 * Returns an opaque timestamp that should be included in future calls
		 * to {@link Service#getNotes(String, String)}.
		 * 
		 * @return
		 */
	...
	static class UserInfoResult implements Serializable {
		...
		/**
		 * Constructs a new response. This constructor can only be invoked on
		 * the server.
		 * 
		 * @param author
		 *            the current author
		...
		/**
		 * Returns the default surface for the author. This is the surface that
		 * will be selected when the application first loads.
		 * 
		 * @return
		 */
	...
	Date changeNotePhoto(String noteKey, int hash, Transformation transformation)
			throws AccessDeniedException;
	...
	/**
	 * Get all notes for the currently logged in author. <code>timestamp</code>
	 * is an opaque timestamp used by the server to optimize the set of results
	 * that are returned. Callers should pass a timestamp from
	 ...
	 * call, or to simply receive the full set of surfaces, pass
	 * <code>null</code> .
	 * 
	 * @param timestamp
	 *            an opaque timestamp
	 ...
	 * @throws AccessDeniedException
	 */
	GetSurfacesResult getSurfaces(String timestamp)
			throws AccessDeniedException;
