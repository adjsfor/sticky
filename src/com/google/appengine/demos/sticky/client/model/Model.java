...
public class Model {
    ...
    private class CreateNoteTask extends Task implements AsyncCallback<CreateObjectResult> {
        ...
        @Override
        public void execute() {
            api.createNote(surface.getKey(), note.getX(), note.getY(), note.getWidth(), note.getHeight(), this);
        }
    ...
    public static void load(final LoadObserver loadObserver, final StatusObserver statusObserver) {
        ...
        api.getUserInfo(new AsyncCallback<Service.UserInfoResult>() {
            ...
            public void onSuccess(UserInfoResult result) {
                loadObserver.onModelLoaded(new Model(result.getAuthor(), result.getSurface(), result.getLogoutUrl(),
                    api, statusObserver));
            }
        });
    }
    ...
    private Model(Author author, Surface selectedSurface, String logoutUrl, ServiceAsync api,
