...
public class Model {
	...
	public interface DataObserver {
         ...
         * 
         */
		void onPhotoTransform(Photo photo, Transformation transformation);
	}
	...
	private class AddAuthorToSurfaceTask extends Task implements
		...
		public void onSuccess(Service.AddAuthorToSurfaceResult result) {
			...
			if (success) {
				surface.update(Model.this, result.getAuthorName(),
						result.getUpdatedAt());
			}
	...
	private class CreateNoteTask extends Task implements
		...
		@Override
		public void execute() {
			api.createNote(surface.getKey(), note.getX(), note.getY(),
					note.getWidth(), note.getHeight(), this);
		}
	...
	private class UpdateNoteContentTask extends Task implements
		...
		public void execute() {
			...
			api.changeNoteContent(note.getKey(), comments, transformation, this);
		}
	...
	public void updateNoteContent(final Note note, List<Comment> comments,
			Transformation transformation) {
		taskQueue
				.post(new UpdateNoteContentTask(note, comments, transformation));
	}
